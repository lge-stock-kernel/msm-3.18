...
static int msm_otg_reset(struct usb_phy *phy)
{
...
#ifdef CONFIG_INPUT_EPACK
	pr_info("%s: reset_counter = %d\n", __func__, motg->reset_counter);
#endif
	...
	if (motg->err_event_seen) {
		dev_err(phy->dev, "performing USB h/w reset for recovery\n");
...
static int msm_otg_suspend(struct msm_otg *motg)
{
	...
	msm_otg_dbg_log_event(phy, "LPM ENTER START",
			motg->inputs, phy->state);
#ifdef CONFIG_INPUT_EPACK
	pr_err("LPM ENTER START, inputs = %lu, phy state = %d, lpm_flags = %lu\n",
			motg->inputs, phy->state, motg->lpm_flags);
#endif
...
#ifdef CONFIG_INPUT_EPACK
	pr_err("host_bus_suspend = %d, device_bus_suspend = %d\n",
...
#ifdef CONFIG_INPUT_EPACK
		pr_info("%s: disable pclk & core_clk\n", __func__);
#endif
		clk_disable_unprepare(motg->pclk);
		clk_disable_unprepare(motg->core_clk);
		if (motg->phy_csr_clk)
...
#ifdef CONFIG_INPUT_EPACK
	pr_err("LPM EXIT START, inputs = %lu, phy state = %d, lpm_flags = %lu\n",
			motg->inputs, phy->state, motg->lpm_flags);
#endif
...
#ifdef CONFIG_INPUT_EPACK
	pr_info("%s: device_bus_suspend = %d\n", __func__, motg->device_bus_suspend);
#endif
...
#ifdef CONFIG_INPUT_EPACK
		pr_info("USB core_clk, pclk enable\n");
#endif
		ret = clk_prepare_enable(motg->core_clk);
		WARN(ret, "USB core_clk enable failed\n");
		ret = clk_prepare_enable(motg->pclk);
...
static void msm_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (motg->adc_state == ADC_STATE_DRY && motg->tm_state == ADC_TM_LOW_STATE)
		delay = 0;
	else
		delay = msecs_to_jiffies(50);

	queue_delayed_work(motg->otg_wq, to_delayed_work(&motg->adc_work.work), delay);
#else
	if (state >= ADC_TM_STATE_NUM) {
		pr_err("%s: invalid notification %d\n", __func__, state);
		return;
				  ...
				  const union power_supply_propval *val)
{
	...
	pr_info("SET PWR PROPERTY, psp = %d, psy->type = %d\n", psp, psy->type);
#endif
