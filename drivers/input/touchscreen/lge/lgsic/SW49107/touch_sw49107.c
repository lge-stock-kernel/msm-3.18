...
#if defined(CONFIG_LGE_TOUCH_CORE_QCT)
			// Called to change DSV(ENM) mode for toggle mode, not power sequence
			lge_panel_exit_deep_sleep();
#endif
...
static int sw49107_suspend(struct device *dev)
{
	...
	// Error handling (Required code)
	if (atomic_read(&ts->state.sleep) == IC_NORMAL && d->lcd_mode == LCD_MODE_U0 &&
		(ts->swipe.mode > SWIPE_NONE || ts->lpwg.mode > LPWG_NONE)) {
