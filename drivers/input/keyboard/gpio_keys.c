	...
	if (type == EV_ABS) {
		...
		*/
		...
		if (!strncmp(bdata->button->desc, "nfc_cover", 9)){
			pr_info("nfc_cover state = %d\n", state);
			if (nfc_ndev.state != !!state) {
				switch_set_state(&nfc_ndev, state);
				pr_info("nfc_cover state changed to %d\n", state);
				...
				const struct gpio_keys_button *button)
{
	...
	if (gpio_is_valid(button->gpio)) {
		...
		if (!strncmp(desc, "nfc_cover", 9)) {
			if (switch_dev_register(&nfc_ndev) < 0) {
				pr_err("nfc_cover switch registration failed\n");
				switch_dev_unregister(&nfc_ndev);
			} else {
				wake_lock_init(&bdata->gpio_irq_wakelock, WAKE_LOCK_SUSPEND, "nfc_ndevc_wakelock");
				pr_info("hall_ic switch registration succeeded\b");
			}
		}
